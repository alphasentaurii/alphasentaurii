...
# Signal Analysis - Star Flux Scatterplots
...
# Scatterplot of Star Flux Signals with Exoplanet
...
To answer the question, I first set out to build a model for the data as is, "sans-physics". The baseline model is a convolutional neural network using the `Keras API` in a `sci-kit learn`-esque wrapper.  
